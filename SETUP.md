...
7. **本番フォームの導通確認 (POST)**:
   ...
   - ブラウザからの手動フォーム送信は **厳密に1回のみ** 実施します:
     - 正常送信テスト（送信完了メッセージの表示、および Gmail 宛先へのメール配信 **1通のみ** を確認）
     - **2回目のブラウザ送信は禁止** です（ブラウザからの通常再送信では新しい `requestId` と Turnstile Token が発行されるため、重複抑止試験にはならず別の問い合わせとして2通目のメールが送信される原因となります）。
     - 同一 `requestId` による重複送信抑止・冪等性検証は自動テスト（TAPテスト）にて担保済みです。
     - Turnstile Token は単回使用（One-time use）であり、本番環境で意図的なリプレイPOSTを行ってはなりません。
     - 万一エラーが発生した場合は安易に再送信せず、直ちに Cloudflare Observability / GAS ログを確認して原因を調査します。
